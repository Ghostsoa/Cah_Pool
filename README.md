...
Cah_Pool(CattleAndHorses)，中文名 **`牛马`** 如果你希望得到一个可以像牛马一样，不断高效的工作的协程池，那么Cah_Pool是一个很好的选择，Cah_Pool是一个高性能的 Go 协程池，原生支持多种优先级任务调度，能够在限定的容量内，自动创建、调度、回收协程，可以极致的压榨每一个协程，并且，所有的调度工作都是每个牛马协程之间自动完成调度，你只需要向他们发送任务即可。提供了简单的 API 和强大的配置选项，以便灵活地应用于不同场景。
